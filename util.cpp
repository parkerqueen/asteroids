...
void closemedia() {
    ...
    Mix_FreeChunk(beat);
    music = NULL;
    fire = explosion = beat = thrust = NULL;
    Mix_Quit();
    SDL_Quit();
}
...
float Rad2Deg(float angle) { return angle * (180.0 / M_PI); }

// seed the random numbers generator by current time (see the documentation of
// srand for further help)...
void InitRandomizer() {
    srand((unsigned int)time(0));  // time(0) returns number of seconds elapsed
...
/*To draw a Circle we need a center point (sx, sy) and the radius, along with
 * the color of circle. This function takes 4 arguments first two arguments (3
 * vertices + 1 color) to draw the triangle with the given color.
 * */
...
void DrawRoundRect(float x, float y, float width, float height, float *color,
    ...
    while (i != segment_count) {
        ...
        bottom_right[index].x = (width - (radius * 2.0f)) +
                                bottom_left_corner.x + (x_offset * radius);
    ...
    static GLubyte clr[] = {156, 207, 255,
                            128};  // Light blue, 50% transparent.
